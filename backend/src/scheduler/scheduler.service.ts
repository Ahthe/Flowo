...
export class SchedulerService {
  ...
  async scheduleTasks(
    ...
    const preferences: any = await this.tasksService.getPreferences(
      token,
      userId,
    );
    if (!preferences) {
    ...
    const daysToSchedule =
      scheduleDto?.daysToSchedule || this.DEFAULT_DAYS_TO_SCHEDULE;
    ...
    activeTasks.sort((a, b) => {
      const pDiff =
        (this.PRIORITY_WEIGHTS[b.priority] || 0) -
        (this.PRIORITY_WEIGHTS[a.priority] || 0);
    ...
    const existingSlots = this.buildTimeSlots(
      tasks.flatMap((t) =>
        (t.instances || []).filter((i: any) => i.status === 'scheduled'),
      ),
    );
  ...
  private async buildPeakHoursMap(
    token: string,
    userId: string,
    tz: string,
  ): Promise<number[]> {
    ...
    try {
      ...
      const { data, error } = await client.rpc('get_user_peak_hours', {
        uid: userId,
        tz,
      });
      if (!error && data) {
      ...
      console.warn(
        'Failed to fetch peak hours via RPC, falling back to empty array',
        e,
      );
    }
    return hours;
  ...
  private fastSchedule(
    ...
    for (const task of tasks) {
      ...
      const existingForTask = existingSlots
      ...
      for (let day = 0; day < days; day++) {
        ...
        const existingToday = taskTimes.filter(
  ...
  private precomputeDayWindows(
    ...
    for (let i = 0; i < days; i++) {
      ...
      const hostHours = preferences.availableHours[dayName] || [
        '09:00',
        '17:00',
      ];
